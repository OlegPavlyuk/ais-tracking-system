# Documentation Index

This is the navigation hub for the AIS Tracking System documentation.

## Architecture

System design, event flow, feature architecture, and technical decisions.

- [Architecture overview](architecture/architecture.md)
- [Architecture decisions](architecture/architecture-decisions.md)
- [Geo validation](architecture/geo-validation.md)
- [Sanctions enrichment](architecture/sanctions-enrichment.md)
- [Realtime delivery](architecture/realtime.md)

## Development

Local setup, testing, APIs, and contributor-facing documentation.

- [Local development](development/local-development.md)
- [Testing](development/testing.md)
- [API reference](development/api.md)

## Operations

Deployment, infrastructure, backups, monitoring, and operational procedures.

- [Deployment](operations/deployment.md)
- [GCP VM runbook](operations/gcp-vm-runbook.md)
- [HTTPS and domain runbook](operations/https-domain-runbook.md)
- [Operations runbook](operations/operations-runbook.md)
- [Restore drill](operations/restore-drill.md)

## Frontend

Frontend-specific notes and diagnostics.

- [Frontend metrics](frontend/frontend-metrics.md)

## Archive

Historical implementation plans and reviews preserved for reference live in [archive/](archive/).
