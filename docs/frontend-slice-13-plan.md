...
`GET /api/vessels` in parallel. Live `position` and `static` events
update an mmsi-keyed Zustand store, which feeds a single MapLibre GeoJSON
source + symbol layer (rAF/100ms-batched `setData`). Pan/zoom changes camera
only; the main marker layer stays store-driven after bootstrap. Reconnect uses
exponential backoff with jitter mirroring the backend AISStream adapter. The
supported coverage bbox is rendered as a
...
| 12a | Significance threshold | Retired by the store-first/global-stream architecture. Pan/zoom no longer changes data ownership. |
| 12b | Debounce | Retired by the store-first/global-stream architecture. No viewport-driven REST or WS update flow remains. |
...
| 13b | Reconnect resync | Always resend `subscribe` on reopen. Additionally fire a REST snapshot only if `Date.now() - disconnectedAt > 5000` ms. Track `disconnectedAt` on socket close. |
| 14 | Error UX | Coverage warning flow removed from the UI. |
| 15 | Initial camera | Temporary static initial camera; coverage-driven framing removed from the frontend. |
...
| 19 | Testing | Vitest + jsdom + `@testing-library/react`. Core tests cover store merge rules, API bootstrap behavior, and backoff behavior. No e2e. |
...
| 22 | Staleness pruning | None on the client in #13. Vessels age out via the next valid bootstrap/resync snapshot or page reload. Backend already filters by `staleMinutes` in `GET /api/vessels`. |
| 23 | Bootstrap ordering | Map `load` -> in parallel: open WS + `subscribe` AND fire REST snapshot. Q11 merge rules are the single consistency mechanism. WS events arriving before snapshot stub-insert; snapshot then merges with timestamp guard. No special bootstrapping mode. |
...
- [x] 2.5 `web/src/api/client.ts`: `fetchVessels(signal)`; throws typed `ApiError` carrying `{ code, message, details }`.
- [x] 2.6 `web/src/lib/backoff.ts`: pure `nextBackoffMs(attempt)` mirroring backend curve + jitter + cap.
- [x] 2.8 `web/src/lib/wsClient.ts`: WS wrapper - URL built from `window.location` (ws/wss); subscribe; Zod-validate inbound; reconnect with backoff; track `disconnectedAt`; onResync callback when outage > 5s.

### Phase 3 — Map + integration

- [x] 3.1 `web/src/map/MapView.tsx`: MapLibre init from `VITE_MAP_STYLE_URL`; register `vessel-default` icon (inline SVG); add `vessels` GeoJSON source + symbol layer.
